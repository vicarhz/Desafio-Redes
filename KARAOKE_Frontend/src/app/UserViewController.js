...
function editarCancion(id){
  localStorage.setItem("id_cancion", id);
  location.href = "./EditarCancion.html";
}
...
async function LlenarTablaCanciones(){
    ...
    for (i = 0; i < canciones.length; i++) {
        $("<tr><td>" + canciones[i].NombreCancion + "</td><td>" + canciones[i].Artista + "</td><td>" +
            ...
            '<input type="image" onclick="editarCancion(\'' + canciones[i]._id.toString() + '\')" title="Edit" width="30" height="30" src="../img/edit.png">'+
            '<input type="image" onclick="borrarCancion(\'' + canciones[i]._id.toString() + '\')" title="Edit" width="30" height="30" src="../img/del.png">'+
...
async function LlenarTablaCancionesFiltrado(){
  ...
  for (i = 0; i < result.length; i++) {
      ...
      result[i].Album + "</td><td>" + result[i].Status + "</td><td>" +
          ...
          '<input type="image" onclick="editarCancion(\'' + result[i]._id.toString() + '\')" title="Edit" width="30" height="30" src="../img/edit.png">'+
          '<input type="image" onclick="borrarCancion(\'' + result[i]._id.toString() + '\')" title="Edit" width="30" height="30" src="../img/del.png">'+
          "</td></tr>").appendTo("#TablaCanciones"); 
  } 
}
...
async function borrarCancion(id){
  ...
  if (confirm('Are you sure you want to delete this song?')) {
    ...
    await borrarCancion_Api(dato);
    await LlenarTablaCanciones();
    alert ( "Was successfully removed!");
  } else {
    //Nada
  }
}
